...
class Net(nn.Module):
    ...
    def __init__(self, seq_len=3, fine_tuning=True):
        ...
        self.classifier = MLP3Net(in_dim=768+4096)
...
def get_data_loader(touch_name, batch_size, p_label='喜楽', k_phase=0):
    test_data_set = manga_data.data[touch_name][
        (manga_data.data[touch_name].story_main_num >= 5) & (manga_data.data[touch_name].original) & (manga_data.data[touch_name].emotion != 'UNK')]
    train_valid_data_set = manga_data.data[touch_name][(manga_data.data[touch_name].story_main_num < 5) & (manga_data.data[touch_name].emotion != 'UNK')]
    # train, valid に分ける
    ...
    koma_vec_train = torch.tensor(train_data_set.koma_vec.values.tolist()).float()
    koma_vec_valid = torch.tensor(valid_data_set.koma_vec.values.tolist()).float()
    koma_vec_test = torch.tensor(test_data_set.koma_vec.values.tolist()).float()

    # 各DataLoaderの準備
    train = TensorDataset(input_ids_train, tok_train, attn_train, koma_vec_train, y_train)
    train_loader = DataLoader(train, batch_size=batch_size, shuffle=True)

    valid = TensorDataset(input_ids_valid, tok_valid, attn_valid, koma_vec_valid, y_valid)
    valid_loader = DataLoader(valid, batch_size=batch_size, shuffle=True)

    test = TensorDataset(input_ids_test, tok_test, attn_test, koma_vec_test, y_test)
...
def get_all_data_loader(batch_size, p_label='喜楽', k_phase=0):
    all_data = manga_data.make_5touch_concat()
    test_data_set = all_data[(all_data.story_main_num >= 5) & (all_data.original) & (all_data.emotion != 'UNK')]
    train_valid_data_set = all_data[(all_data.story_main_num < 5) & (all_data.emotion != 'UNK')]
    # train, valid に分ける
    train_data_set, valid_data_set = train_test_split(
        train_valid_data_set,
        test_size=0.2,
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, p_label, batch_size=16, epochs=200, bairitsu=1):
        ...
        self.train_loader, self.valid_loader, self.test_loader, self.test_data_set, self.w = \
            get_all_data_loader(batch_size=self.batch_size)
        ...
        self.log_path = './result_5touch_' + P_DIC[self.p_label] + '_hotto.txt'
        self.new_model_path = '../models/bert/My_Japanese_transformers/' + '5touch' + '_' + P_DIC[self.p_label] + '_hotto.bin'
    ...
    def set_dataset(self, k_phase=0):
        self.train_loader, self.valid_loader, self.test_loader, self.test_data_set, self.w = \
            get_all_data_loader(self.batch_size)

        self.data_loaders = {'train': self.train_loader, 'valid': self.valid_loader}

    def manga4koma_train(self, lr, is_study=False, k_phase=0):
        self.is_study = is_study

        self.history = History()
        self.net = Net(seq_len=6).to(device)

        self.set_dataset(k_phase)
        ...
        for epoch in range(self.epochs):
            ...
            for phase in ['train', 'valid']:
                ...
                for x_input_ids, x_tok, x_attn, koma, y in self.data_loaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        y_pred = self.net(input_ids=x_input_ids, token_type_ids=x_tok, attention_mask=x_attn, koma_vec=koma)
                ...
                # if (epoch == 0) & (phase == 'valid') & (self.is_study is False):
                #     self.save_model()
                # if (phase == 'valid') & (self.history.enable_save) & (self.is_study is False):
                #     self.save_model()

                if (epoch == 0) & (phase == 'valid'):
                    self.save_model()
                elif (phase == 'valid') & (self.history.enable_save):
                    self.save_model()
    ...
    def manga4koma_test(self):
        ...
        self.net.eval()
        with torch.no_grad():
            for x_input_ids, x_tok, x_attn, koma, y_test in self.test_loader:
                x_input_ids = Variable(x_input_ids).to(device)
                x_tok = Variable(x_tok).to(device)
                x_attn = Variable(x_attn).to(device)
    ...
    def manga4koma_test_by_touch(self, touch_name):
        p_label = '喜楽'
        test_data_set = self.test_data_set[self.test_data_set.touch == touch_name]
        y_test = np.identity(2)[[0 if emo == p_label else 1 for emo in test_data_set.emotion]]
    ...
    def objective_variable(self):
        ...
        def objective(trial):
            ...
            best_valid_f1 = self.manga4koma_train(lr=lr, is_study=False)
    ...
    for cnt in range(4):
        for p_emo in P_EMOTIONS:
            ...
            for touch_name in ['5touch']:
                ...
                for touch in TOUCH_NAME_ENG:

                    ex.manga4koma_test_by_touch(touch)
