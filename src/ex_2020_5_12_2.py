...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.classifier = MLP3Net(300, 30, 2)
...
def get_data_loader(touch_name, batch_size, d2v_model_name):
    manga_data.data[touch_name].wakati = pd.read_pickle('dataset/' + touch_name + '_emb_' + d2v_model_name + '.pkl')
    ...
    X_train = torch.tensor(train_data_set.wakati.values.tolist())
    y_train = torch.tensor(y_train, requires_grad=True).long()

    X_valid = torch.tensor(valid_data_set.wakati.values.tolist())
    y_valid = torch.tensor(y_valid, requires_grad=True).long()

    X_test = torch.tensor(test_data_set.wakati.values.tolist())
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, d2v_model_name='manga109', batch_size=16, epochs=200):
        self.epochs = epochs
        self.batch_size = batch_size
        self.d2v_model_name = d2v_model_name
        ...
        self.log_path = './result_' + self.touch_name + '_' + self.d2v_model_name + '.txt'
        self.new_model_path = './model/' + self.touch_name + '_' + self.d2v_model_name + '_model.bin'
...
def main():
    ...
    for touch_name in TOUCH_NAME_ENG:
        ...
        for d2v in ['manga109', 'wiki_mini', 'wiki_mini_manga109']:

            ex = Manga4koma_Experiment(touch_name=touch_name, d2v_model_name=d2v, batch_size=16, epochs=200)
