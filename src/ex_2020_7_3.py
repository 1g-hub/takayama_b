...
class Net(nn.Module):
    ...
    def __init__(self, seq_len=3, fine_tuning=True):
        ...
        self.classifier = MLP3Net(in_dim=128)
    ...
    def forward(self, x):
        ...
        _, out = self.lstm(bert_out)
        # lstm_out[0]は３次元テンソルになってしまっているので2次元に調整して全結合
        out = self.classifier(out[0].view(-1,128))
        return out
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, p_label, batch_size=16, epochs=200):
        ...
        self.log_path = './result_' + self.touch_name + '_' + P_DIC[self.p_label] + '_seq_len5_last_lstmmlp.txt'
        self.new_model_path = '../models/bert/My_Japanese_transformers/' + self.touch_name + '_' + P_DIC[self.p_label] + '_seq_len5_last_lstmmlp.bin'
    ...
    def objective_variable(self):
        ...
        def objective(trial):
            lr = trial.suggest_loguniform('lr', 8e-8, 8e-7)
