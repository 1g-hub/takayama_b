...
# ========
# GLOBAL 変数
TOUCH_NAME_ENG = ['moe',"shounen",'seinen']

P_EMOTIONS = ['喜楽']

P_DIC = {'ニュートラル':'neutral', '驚愕':'kyougaku', '喜楽':'kiraku'}

manga_data = manga4koma(to_zero_pad=True, to_sub_word=True, to_sequential=True, seq_len=4)
...
class Net(nn.Module):
    ...
    def __init__(self, seq_len=3, fine_tuning=True):
        ...
        self.bert_encoder = BertModel.from_pretrained(
            '../models/bert/Japanese_L-12_H-768_A-12_E-30_BPE_WWM_transformers/pytorch_model.bin',
            config=self.config)
        self.bi_lstm = self_net.BiLSTMEncoder(1536, 128)
        self.classifier = self_net.SelfAttentionClassifier(128, 64, 3, 2)

        self.koma_lstm = nn.LSTM(input_size=4096,
                            hidden_size=768,
    ...
    def forward(self, x, koma_vec):
        ...
        koma_vec, _ = self.koma_lstm(koma_vec)
        koma_vec = koma_vec.permute(1, 0, 2)
        # print("before bert shape : {}".format(x.size()))
        bert_out = torch.empty(self.seq_len, batch_size, 1536).to(device)
        for i in range(self.seq_len):
            ...
            bert_out[i] = torch.cat([self.bert_encoder(x[i])[0][:, 0, :], koma_vec[i]], dim=1)
...
def get_data_loader(touch_name, batch_size, p_label='喜楽'):
    ...
    # 各DataLoaderの準備
    train = TensorDataset(X_train, koma_vec_train, y_train)
    train_loader = DataLoader(train, batch_size=batch_size, shuffle=True)

    valid = TensorDataset(X_valid, koma_vec_valid, y_valid)
    valid_loader = DataLoader(valid, batch_size=batch_size, shuffle=True)

    test = TensorDataset(X_test, koma_vec_test, y_test)
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, p_label, batch_size=16, epochs=200):
        ...
        self.log_path = './result_' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_lstm_seq4_bert_last_layer.txt'
        self.new_model_path = '../models/bert/My_Japanese_transformers/' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_lstm_seq4_bert_last_layer.bin'
    ...
    def manga4koma_train(self, lr, is_study=False):
        ...
        self.net = Net(seq_len=4).to(device)
