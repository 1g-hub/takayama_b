...
class Net(nn.Module):
    ...
    def __init__(self, fine_tuning=True):
        ...
        self.bert_encoder = BertModel.from_pretrained('../models/bert/Japanese_L-12_H-768_A-12_E-30_BPE_WWM_transformers/pytorch_model.bin',
                                              config=self.config)
        self.classifier = MLP3Net(in_dim=1536)

        self.koma_linear = ClassificationNet(in_dim=4096, out_dim=768)
    ...
    def forward(self, x, koma_vec):
        ...
        koma_vec = self.koma_linear(koma_vec)
...
def get_data_loader(touch_name, batch_size, p_label='喜楽'):
    ...
    koma_vec_train = torch.tensor(train_data_set.koma_vec.values.tolist())
    koma_vec_valid = torch.tensor(valid_data_set.koma_vec.values.tolist())
    koma_vec_test = torch.tensor(test_data_set.koma_vec.values.tolist())
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, p_label, batch_size=16, epochs=200):
        ...
        self.log_path = './result_' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_komalinear_bert_last_layer.txt'
        self.new_model_path = '../models/bert/My_Japanese_transformers/' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_komalinear_bert_last_layer.bin'
    ...
    def objective_variable(self):
        ...
        def objective(trial):
            lr = trial.suggest_loguniform('lr', 4e-7, 1e-6)
