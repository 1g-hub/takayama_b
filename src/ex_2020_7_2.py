...
class Net(nn.Module):
    ...
    def __init__(self, seq_len=3, fine_tuning=True):
        ...
        self.bert_encoder = BertModel.from_pretrained(
            ...
            config=self.config)
        self.bi_lstm = self_net.BiLSTMEncoder(4864, 128)
    ...
    def forward(self, x, koma_vec):
        ...
        bert_out = torch.empty(self.seq_len, batch_size, 4864).to(device)
...
class Manga4koma_Experiment():
    def __init__(self, touch_name, p_label, batch_size=16, epochs=200):
        ...
        self.log_path = './result_' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_seq3_fix.txt'
        self.new_model_path = '../models/bert/My_Japanese_transformers/' + self.touch_name + '_' + P_DIC[self.p_label] + '_multi_seq3_fix.bin'
    ...
    def objective_variable(self):
        ...
        def objective(trial):
            lr = trial.suggest_loguniform('lr', 5e-8, 3e-7)
